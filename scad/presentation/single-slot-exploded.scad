// Deterministic single-slot exploded source for issue #32.
// Presentation-only caller: all product geometry comes from the pinned modules.

use <../casing.scad>;
use <../tub.scad>;
use <../knob.scad>;
use <../peg.scad>;

$fn = 64;

PITCH_Z = 3.5;
MINIMUM_LIP_Z = 3.74;
TUB_EXTERIOR_X = 256.4;
TUB_EXTERIOR_Y = 340.4;
TUB_SOURCE_HEIGHT_U = 23;
TUB_EXTERIOR_Z = TUB_SOURCE_HEIGHT_U * PITCH_Z + MINIMUM_LIP_Z;
SIDE_CLEARANCE = 1.0;
BACK_CLEARANCE = 0.5;
TOP_CLEARANCE = 1.0;
SIDE_THICKNESS = 25;
BACK_THICKNESS = 25;
TOP_THICKNESS = 10;
PEG_SPACING = 190;
PEG_DIAMETER = 8.0;
PEG_CLEARANCE = 0.45;
PEG_CHAMFER = 2.5;
PEG_SOCKET_EDGE_LAND = 4.0;

SLOT_WIDTH = TUB_EXTERIOR_X + 2 * SIDE_CLEARANCE;
SLOT_DEPTH = TUB_EXTERIOR_Y + BACK_CLEARANCE;
SLOT_HEIGHT = TUB_EXTERIOR_Z + TOP_CLEARANCE;
OUTER_X = SLOT_WIDTH + 2 * SIDE_THICKNESS;
OUTER_Y = SLOT_DEPTH + BACK_THICKNESS;
PRINT_Z = TOP_THICKNESS + SLOT_HEIGHT;
TUB_CENTER_X = SIDE_THICKNESS + SIDE_CLEARANCE + TUB_EXTERIOR_X / 2;
TUB_CLOSED_CENTER_Y = TUB_EXTERIOR_Y / 2;
TUB_SLOT_BOTTOM_Z = -TOP_THICKNESS - TOP_CLEARANCE - TUB_EXTERIOR_Z;
HANDLE_CENTER_Z = 36;

TUB_EXPLODE_Y = -205;
TUB_EXPLODE_Z = -92;
KNOB_EXPLODE_Y = -430;
KNOB_EXPLODE_Z = -145;
PEG_EXPLODE_Z = 72;
PRODUCT_SCALE = 1;
INCLUDE_CASING = true;
INCLUDE_TUB = true;
INCLUDE_KNOB = true;
INCLUDE_PEGS = true;

CASING_COLOR = [0.74, 0.77, 0.80, 1.0];
TUB_COLOR = [0.32, 0.52, 0.63, 1.0];
KNOB_COLOR = [0.18, 0.20, 0.22, 1.0];
PEG_COLOR = [0.72, 0.54, 0.30, 1.0];

assert(PRODUCT_SCALE == 1, "HF_HARD_REJECT:SCALE_CHANGED");

function presentation_peg_end_inset() =
    peg_socket_end_inset(
        SIDE_THICKNESS,
        BACK_THICKNESS,
        PEG_DIAMETER,
        PEG_CLEARANCE,
        PEG_CHAMFER,
        PEG_SOCKET_EDGE_LAND
    );

function presentation_peg_positions() =
    peg_reference_positions(
        OUTER_X,
        OUTER_Y,
        SIDE_THICKNESS,
        BACK_THICKNESS,
        PEG_SPACING,
        presentation_peg_end_inset()
    );

module installed_casing() {
    echo("HF_PART|casing|1");
    color(CASING_COLOR)
        translate([OUTER_X, 0, 0])
            rotate([0, 180, 0])
                hutchfinity_casing(
                    slot_width = SLOT_WIDTH,
                    slot_depth = SLOT_DEPTH,
                    slot_height = SLOT_HEIGHT,
                    side_thickness = SIDE_THICKNESS,
                    back_thickness = BACK_THICKNESS,
                    top_thickness = TOP_THICKNESS,
                    peg_spacing = PEG_SPACING,
                    casing_magnet_drawer_depth = TUB_EXTERIOR_Y
                );
}

module exploded_tub() {
    echo("HF_PART|tub|1");
    color(TUB_COLOR)
        translate([
            TUB_CENTER_X,
            TUB_CLOSED_CENTER_Y + TUB_EXPLODE_Y,
            TUB_SLOT_BOTTOM_Z + TUB_EXPLODE_Z
        ])
            hutchfinity_regular_tub();
}

module exploded_knob() {
    echo("HF_PART|knob|1");
    color(KNOB_COLOR)
        translate([
            TUB_CENTER_X,
            KNOB_EXPLODE_Y,
            TUB_SLOT_BOTTOM_Z + HANDLE_CENTER_Z + KNOB_EXPLODE_Z
        ])
            rotate([90, 0, 0])
                hutchfinity_knob();
}

module exploded_pegs() {
    for (p = presentation_peg_positions()) {
        echo("HF_PART|peg|1");
        color(PEG_COLOR)
            translate([p[0], p[1], PEG_EXPLODE_Z])
                hutchfinity_peg();
    }
}

echo("HF_SCENE|single-slot-exploded");
echo(str("HF_META|product_scale|", PRODUCT_SCALE));
echo(str("HF_META|authoritative_peg_count|", len(presentation_peg_positions())));

scale([PRODUCT_SCALE, PRODUCT_SCALE, PRODUCT_SCALE]) {
    if (INCLUDE_CASING) installed_casing();
    if (INCLUDE_TUB) exploded_tub();
    if (INCLUDE_KNOB) exploded_knob();
    if (INCLUDE_PEGS) exploded_pegs();
}
